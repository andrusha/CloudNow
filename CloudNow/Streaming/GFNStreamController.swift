...
final class GFNStreamController: NSObject {
    ...
    nonisolated private static func parseConnectionStats(
        ...
        let selected = selectedCandidatePairID.flatMap { candidatePairs[$0] }
            ?? candidatePairs.values.first(where: \.nominated)
            ?? candidatePairs.values.first
        guard let selected else { return nil }
        let local = candidateDetails[selected.localID]
        let remote = candidateDetails[selected.remoteID]
